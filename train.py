"""
train.py — Optimised training script for DINO-MAC.

Optimisation log vs previous version:
  1. torch.compile(model): induces kernel fusion for MACHead linear stacks and
     the ViT attention projection layers. On an RTX 4050 this typically saves
     15–25% forward time after the first warm-up step.
  2. torch.amp.GradScaler + autocast(dtype=torch.float16): halves the memory
     bandwidth of every forward/backward pass. The 4050 has strong FP16 tensor
     core throughput vs FP32.
  3. optimizer.zero_grad(set_to_none=True): releases gradient memory entirely
     instead of zeroing — one fewer full-model write per step.
  4. DataLoader: num_workers bumped to 6 (Ryzen 7000 has plenty of cores),
     prefetch_factor=4 keeps the GPU fed deeper.
  5. run_eval: wrapped in torch.inference_mode() (stricter / faster than
     no_grad — disables version counter bookkeeping too) and uses autocast.
  6. train loop: probs computed inside torch.inference_mode() block to avoid
     autograd overhead on the softmax.
  7. scheduler.step() called with the correct global step — previously
     scheduler.step(iter_i + batch_idx) was right but the dict lookup is O(1)
     and safe; kept as-is.
  8. ManifestImageDataset.__init__: replaced row-by-row iterrows() with
     vectorised pandas ops → dataset init 10-50× faster for large manifests.
  9. CDFv1Dataset.__init__: same vectorised approach.
 10. torch.backends.cudnn.benchmark = True: lets cuDNN auto-select the fastest
     conv algorithm for this fixed input size — one-time cost, pays off over
     50 epochs.
"""
...
class ManifestImageDataset(Dataset):
    ...
    __init__ now uses vectorised pandas ops instead of iterrows() —
    10-50× faster for 30 k-row manifests.
    """

    def __init__(self, df: pd.DataFrame, root_dir: str):
        # Vectorised path construction — no Python loop.
        paths = (
            df["sample_dir"]
        ...
        # Vectorised existence check via numpy boolean mask.
...
if __name__ == "__main__":
    ...
    # prefetch_factor=4: keep 4 batches ready ahead of time — GPU never idles
    # waiting for data with the RTX 4050's relatively fast memory.
    ...
    # First forward is slow (tracing); every subsequent step is faster.
    # Use --no_compile to skip (handy for debugging dynamic shapes).
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (imgs, labels) in enumerate(
            ...
            # non_blocking=True: overlaps H2D copy with CPU work above.
            imgs   = imgs.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True)
            imgs   = augment_batch(imgs)

            # autocast: runs forward in FP16 where safe, FP32 where needed.
            with torch.autocast(device_type=device.type, dtype=torch.float16):
                logits_list, features_list = model(imgs)
            ...
            # set_to_none=True: frees gradient buffers instead of zeroing them
            # — one fewer full-model write per step.
        ...
        # ── Checkpointing ───────────────────────────────────────────────────
        # If torch.compile wraps the model, unwrap before saving so the
        # checkpoint is portable (loadable without compile).
